...
INTEGER CODING RULES:
  ...
  must conform to the following style: 严格按照下面的格式：
  ...
  Each "Expr" is an expression using ONLY the following: 表达式只能包含这些：
  1. Integer constants 0 through 255 (0xFF), inclusive. You are
      not allowed to use big constants such as 0xffffffff. 0-255 的整数
  2. Function arguments and local variables (no global variables). 无全局变量
  3. Unary integer operations ! ~ 一元运算法
  4. Binary integer operations & ^ | + << >> 二元运算符
  ...
  1. Uses 2s complement, 32-bit representations of integers. 32 位整数
  2. Performs right shifts arithmetically. 算数右移
  3. Has unpredictable behavior when shifting if the shift amount
     is less than 0 or greater than 31. 注意异常位移
...
NOTES:
  ...
  2. Each function has a maximum number of operations (integer, logical,
     ...
     these as you want without penalty. 运算符有上限
